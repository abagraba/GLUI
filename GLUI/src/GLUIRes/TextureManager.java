...
public class TextureManager {
	...
	public static void useTexture(String textureName) {
		Texture t = ResourceManager.getTexture(textureName);
		if (t == null) {
			Debug.log("Cannot use texture. " + textureName + " not found by ResourceManager.");
			useNone();
			return;
		}
		if (!t.equals(boundTexture)) {
			...
			if (textures.containsKey(t)) {
				...
				Debug.log("Cannot use texture. " + textureName + " failed to initialize.");
		}
	}
	...
	public static void useTexture(Texture texture) {
		...
		if (!texture.equals(boundTexture)) {
			...
			if (textures.containsKey(texture)) {
				...
				glBindTexture(texture.getGLType(), textures.get(texture));
			}
			else
				Debug.log("Cannot use texture. " + texture.name + " failed to initialize.");
		}
	}
